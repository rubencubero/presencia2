...
app.get('/activos_ccoste/:cCoste', async (req, res, next) => {
  ...
  const [rows] = await db.query(`SELECT pr.id AS idparte, pt.descri, p.* FROM presencia pr LEFT JOIN puesto_trabajo pt ON pr.idpdt = pt.id 
...
/* Hacks para arreglar */
async function actualizarIDPuesto(){ return await db.query("UPDATE presencia SET idpda = 75 WHERE USER = 'PDP01';");};
async function actualizarEstado0(){ return await db.query('UPDATE presencia SET estado = 314 WHERE estado = 0');};
...
async function cerrarPartesDesdeSistema(){  
  ...
  for (i=0;i<rows.length;i++){
    ...
    cerrarParte(rows[i].id, 284, Buffer.from('Informática: Registro duplicado','utf-8').toString(), rows[i].inicio);
  }
  [rows] = [];
  ...
  for (i=0;i<rows.length;i++){
    ...
    cerrarParte(rows[i].id, 284, Buffer.from('Informática: Cierre automático por 13 horas','utf-8').toString());
  }
  [rows] = [];
}
...
async function getPartesDuplicados(){  return await db.query('SELECT MAX(id) AS id, COUNT(id) as contador, MAX(inicio) as inicio FROM presencia WHERE (fin = "" OR fin IS NULL) GROUP BY idpersona, inicio HAVING contador > 1 ORDER BY id ASC;');}
...
async function cerrarParte(id, tipo, observacion, fechaFin = ''){  
  ...
  logInfo("UPDATE presencia SET fin = '" + fechaFin + "', idtlogout = " + tipo + ", observaciones = '" + observacion + "' WHERE id = " + id + ";");
  const result = await db.query("UPDATE presencia SET fin = '" + fechaFin + 
                                "', idtlogout = " + tipo 
